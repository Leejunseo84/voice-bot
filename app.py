...
def main():
    ...
    # session_state 관리: 서버의 session에 사용자 변수를 저장
    ...
    with col1:
        ...
        if (audio.duration_seconds > 0) and not st.session_state['check_reset']:
            st.audio(audio.export().read()) # 녹음된 음원파일을 화면상에서 재생 
            ...
            # tts 변환 (이진 파일로 받기 위함)
            base64_encoded_audio:str = tts(response) # 이진 데이터를 base64 인코딩한 결과(문자열)
            st.html(f'''
            <audio autoplay='true'>
            ...
            st.session_state['check_reset'] = False # 화면 초기화 후 다시 check_reset False 지정
